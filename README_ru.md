# Nmap: определение отпечатков и скриптовый движок

**Языки:** [English](README.md) · [简体中文](README_zh.md) · [Español](README_es.md) · [Français](README_fr.md) · [Deutsch](README_de.md) · [日本語](README_ja.md) · [Русский](README_ru.md) · [한국어](README_ko.md) · [Português](README_pt.md)

<p align="center">
  <a href="https://labex.io/ru/courses/nmap-fingerprinting-the-scripting-engine">
    <img src="https://course-cover.labex.io/nmap-fingerprinting-the-scripting-engine.png?lang=ru" alt="Nmap: определение отпечатков и скриптовый движок">
  </a>
</p>

Совершенствуйте свои навыки работы с Nmap, освоив определение версий служб и отпечатков ОС. Используйте скриптовый движок Nmap (NSE) для автоматизации поиска уязвимостей и аудита сетевых служб на наличие критических недостатков.

[Начать курс на LabEx](https://labex.io/ru/courses/nmap-fingerprinting-the-scripting-engine)

## Упражнения

|   Индекс | Название                                              | Сложность   | Практика                                                                                                                                                                |
|----------|-------------------------------------------------------|-------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|        1 | Обнаружение служб и версий                            | Начинающий  | <a target='_blank' href='https://labex.io/ru/labs/linux-service-and-version-detection-656142?course=nmap-fingerprinting-the-scripting-engine'>Начать лабораторию</a>    |
|        2 | Идентификация операционной системы (OS Fingerprint... | Начинающий  | <a target='_blank' href='https://labex.io/ru/labs/linux-operating-system-fingerprinting-656141?course=nmap-fingerprinting-the-scripting-engine'>Начать лабораторию</a>  |
|        3 | Основы Nmap Scripting Engine (NSE)                    | Начинающий  | <a target='_blank' href='https://labex.io/ru/labs/linux-nmap-scripting-engine-nse-basics-656140?course=nmap-fingerprinting-the-scripting-engine'>Начать лабораторию</a> |
|        4 | Сканирование уязвимостей с помощью NSE                | Начинающий  | <a target='_blank' href='https://labex.io/ru/labs/linux-vulnerability-scanning-with-nse-656143?course=nmap-fingerprinting-the-scripting-engine'>Начать лабораторию</a>  |
|        5 | Задача по перечислению целей                          | Средний     | <a target='_blank' href='https://labex.io/ru/labs/linux-target-enumeration-challenge-656139?course=nmap-fingerprinting-the-scripting-engine'>Начать испытание</a>       |

## About LabEx

<div align="left"><p><a href="https://labex.io"><strong>LabEx</strong></a> is a <strong>hands-on learning platform for beginners</strong>.</p><p>Explore <a href="https://labex.io/learn/linux"><strong>Linux</strong></a>, <a href="https://labex.io/learn/devops"><strong>DevOps</strong></a>, <a href="https://labex.io/learn/cybersecurity"><strong>Cybersecurity</strong></a>, and <strong>more</strong> — all directly in your browser.</p><p>Learn step by step through <strong>interactive labs</strong>, <strong>guided exercises</strong>, and <strong>real-world projects</strong>. 🌱<br />No setup, no stress — just practice and grow your skills by doing.</p><br /><p><a href="https://apps.apple.com/app/id6765840991"><img src="https://developer.apple.com/assets/elements/badges/download-on-the-app-store.svg" alt="Download on the App Store" height="54" /></a>&nbsp;<a href="https://apps.apple.com/app/id6765840991"><img src="https://developer.apple.com/app-store/marketing/guidelines/images/badge-download-on-the-mac-app-store.svg" alt="Download on the Mac App Store" height="52" /></a></p><br /><p>📖 Need help? Visit our <a href="https://support.labex.io/">Help Center</a> or email info@labex.io</p></div>

